...
public partial class IndexDetailTabView : UserControl
{
    private TextEditor? _ddlEditor;
    private IndexDetailTabViewModel? _currentVm;

    public IndexDetailTabView()
    {
        InitializeComponent();
    ...
    private void OnDataContextChanged(object? sender, EventArgs e)
    {
        ...
        _currentVm = DataContext as IndexDetailTabViewModel;
        if (_currentVm is not null)
        {
            _currentVm.PropertyChanged += OnVmPropertyChanged;
            PushDdl();
    ...
    {
        if (e.PropertyName == nameof(IndexDetailTabViewModel.DdlText))
        {
            PushDdl();
        }
    }
