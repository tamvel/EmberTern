...
public partial class DomainDetailTabView : UserControl
{
    private TextEditor? _ddlEditor;
    private DomainDetailTabViewModel? _currentVm;

    public DomainDetailTabView()
    {
        InitializeComponent();
    ...
    private void OnDataContextChanged(object? sender, EventArgs e)
    {
        ...
        _currentVm = DataContext as DomainDetailTabViewModel;
        if (_currentVm is not null)
        {
            _currentVm.PropertyChanged += OnVmPropertyChanged;
            PushDdl();
    ...
    {
        if (e.PropertyName == nameof(DomainDetailTabViewModel.DdlText))
        {
            PushDdl();
        }
    }
