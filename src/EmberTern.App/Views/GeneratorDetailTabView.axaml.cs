...
public partial class GeneratorDetailTabView : UserControl
{
    private TextEditor? _ddlEditor;
    private GeneratorDetailTabViewModel? _currentVm;

    public GeneratorDetailTabView()
    {
        InitializeComponent();
        _ddlEditor = this.FindControl<TextEditor>("GeneratorDdlEditor");
        ApplyEditorTheme();
    ...
    private void OnDataContextChanged(object? sender, EventArgs e)
    {
        ...
        _currentVm = DataContext as GeneratorDetailTabViewModel;
        if (_currentVm is not null)
        {
            _currentVm.PropertyChanged += OnVmPropertyChanged;
            PushDdl();
    ...
    {
        if (e.PropertyName == nameof(GeneratorDetailTabViewModel.DdlText))
        {
            PushDdl();
        }
    }
