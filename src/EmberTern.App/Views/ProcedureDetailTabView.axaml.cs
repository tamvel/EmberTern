...
public partial class ProcedureDetailTabView : UserControl
{
    ...
    public ProcedureDetailTabView()
    {
        ...
        if (_inputGrid is not null) BuildFieldColumns(_inputGrid, includeDefault: true);
        if (_outputGrid is not null) BuildFieldColumns(_outputGrid, includeDefault: false);
        if (_variablesGrid is not null) BuildFieldColumns(_variablesGrid, includeDefault: true);
    ...
    // ─── Full field-definition grid columns (Input / Output / Variables) ───
    //
    // Built once in code so the 12-column field model (Name / Type combo / TYPE OF /
    // Domain combo / Size / Scale / Sub Type / Charset / Not Null / Collate / Default /
    // Description) is defined in ONE place and shared by all three grids — same field
    // infrastructure as the table grids, no second type system. Output omits Default.

    private static void BuildFieldColumns(DataGrid grid, bool includeDefault)
    {
        grid.Columns.Clear();
