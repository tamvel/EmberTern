...
| **Headless session init race** | ⭐ **Characterised 2026-08-14 — no longer "three candidate causes, none proven".** `HeadlessUnitTestSession` builds its Avalonia application lazily inside the first `Dispatch`; `AvaloniaHeadlessPlatform.Initialize` → `Compositor` → `DefaultRenderLoop.Add` → `Dispatcher.VerifyAccess()` intermittently throws *"the calling thread cannot access this object"*. **Whichever headless test dispatches first is the one that dies**, so the failing test name changes every run — which is why it read as several unrelated flakes. Measured rate: **~2 runs in 5**, costing **1 test**. ⛔ An eager warm-up in the fixture ctor was tried and **reverted**: it did not change the rate and turned the 1-test loss into a **375-test** loss (a throwing collection fixture fails the whole collection). The partitioned run used to hide this. Its own infrastructure task; needs Avalonia's headless dispatcher/scope contract established. |
