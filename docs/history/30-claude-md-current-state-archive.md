# 30 — `CLAUDE.md` § "Current state" — verbatim archive (2026-07-12 → 2026-08-10)

> **Provenance.** This file is a **verbatim extract** of the `## Current state` section of
> `CLAUDE.md` as it stood at commit `00931a2` (2026-08-11), immediately before the second
> **Documentation Cleanup Sprint**. The prose is copied **byte-for-byte**: nothing was rewritten,
> summarized, reordered or dropped — the same rule the first cleanup sprint (2026-07-11)
> applied to `docs/history/01`–`20` (see `docs/history/README.md`, §0 "never lose information").
>
> ⚠ **One disclosed deviation, mechanical and prose-free: markdown link TARGETS were re-based.**
> The text was authored for a file at the **repo root**, so its 74 links read `](docs/…)` / `](src/…)`;
> from `docs/history/` those resolve nowhere. They are now `](../../docs/…)` / `](../../src/…)`, and the
> six pointing at stage handovers were re-pointed to `docs/history/handovers/`, where this sprint moved
> them. Verified by a prose-integrity check: with every link target masked, the file is byte-identical
> to the extract. ⭐ The alternative — leaving 74 dead links in an archive nobody can navigate — would
> have preserved the letter of "verbatim" and lost its purpose.
>
> **Why it was moved.** The section had grown to **5 956 lines / 605 681 characters — 83 % of
> `CLAUDE.md`** and was loaded into *every* session automatically. It is a chronological diary of
> **78 entries** covering 2026-07-12 → 2026-08-10, and nearly every entry is marked
> `ZAMKNIĘTY` / `CLOSED` / `DONE` / `MERGED`. The first sprint had cut `CLAUDE.md` from 4 495 to
> **472** lines; by 2026-08-10 it was back to **6 849** — 14,5× regrowth in 30 days, i.e. the same
> defect recurring. Moving the diary out is the fix; keeping it verbatim is what makes the fix safe.
>
> ⭐ **This is a SECOND COPY, not the only record.** Every stage described below also has a
> dedicated, kept-current document — verified before the move:
>
> | Stage described below | Its own document |
> |---|---|
> | Localization / App + Core (C0–C8) | `docs/design/localization.md` · `docs/history/28-localization-core-stage.md` |
> | Product Polish M0–M5 (incl. M4/M5 iterations) | `docs/design/product-polish.md` §19 · `docs/design/color-language.md` |
> | Post-M5 UX package (points 1–6) | `docs/history/27-post-m5-ux-package.md` |
> | M5 / M‑4 terminology · DPI | `docs/design/terminology.md` · `docs/design/product-polish-m5-dpi-checklist.md` |
> | Firebird Debugger (P1/P2, D1–D15, Draft model) | `docs/design/firebird-debugger.md` · `…-implementation-plan.md` · `docs/history/19-firebird-debugger.md` |
> | Settings Center & formatter casing | `docs/design/settings-center.md` |
> | Keyboard Manager / command system | `docs/design/keyboard-manager.md` |
> | Hamburger navigation / About / notices | `docs/design/hamburger-navigation.md` |
> | Data Import | `docs/design/data-import.md` · `docs/history/21-data-import.md` |
> | Editor front-end, Stage 7, Stage Q, Stage 8 | `docs/design/editor-architecture.md` · `…-stage7-diagnostics.md` · `…-quick-fixes.md` · `docs/history/14`, `16`, `17`, `18`, `20` |
> | Stabilization / grid / language sprints | `docs/history/24`, `25`, `26` |
> | Avalonia 12.1.1 update | `docs/design/avalonia-12.1.1-update.md` |
> | Formatter + PSQL DML binding fix | `docs/history/29-formatter-and-psql-dml-binding-fix.md` |
> | Architecture hardening | `docs/history/22-architecture-hardening-sprint.md` |
> | Metadata refresh | `docs/design/metadata-refresh-analysis.md` |
>
> ⚠⚠ **Read it as a DATED RECORD, never as current state.** Several claims below were already
> false when this file was created, and are corrected in `docs/current-state.md`:
> `feat/product-polish` is described as unmerged (it **is** merged into `master`); the localization
> branch is described as *"⛔ NIE pushowana, NIE scalona"* (it **was** merged as `00931a2`); the test
> count appears as 8345 → 8392 → 8467 → 8730 in four layered corrections (the real figure is
> **8 768**); the headless partition is said to hold 22 class names (**24**, and the rule is to
> derive it, never transcribe it); and the Polish translation is listed as outstanding (**delivered**,
> `Strings.pl.resx`, 2 409 entries).
>
> ⭐ **For the current state read `docs/current-state.md`. For the rules read `CLAUDE.md`.**
> This file answers only *"what happened, and why did we decide it that way"*.
...
- **🔒🌍 LOCALIZATION / APP — ETAP ZAMKNIĘTY I ODEBRANY (2026-08-09).** Gałąź
  ...
  **[docs/design/localization-app-stage-handover.md](../../docs/history/handovers/localization-app-stage-handover.md)**.
...
- **🏁🔒 M5 · PRODUCT POLISH — ETAP ZAMKNIĘTY W CAŁOŚCI I ODEBRANY (2026-08-10). ⭐ WRAZ Z NIM ZAMKNIĘTY
  ...
  **[product-polish-m5-next-session.md](../../docs/history/handovers/product-polish-m5-next-session.md)**.
...
- **🔤🔒 M4 · BLOK TYPOGRAFII — ZAMKNIĘTY, ODEBRANY PO QA WIZUALNYM UŻYTKOWNIKA (2026-08-08).**
  ...
  **[docs/design/product-polish-m4-migration-next-session.md](../../docs/history/handovers/product-polish-m4-migration-next-session.md)**.
...
- **🎨 PRODUCT POLISH — ACTIVE STAGE. Branch `feat/product-polish`. M3 · M3b · ⛔ the §13.3 GATE · M3.5 are all
  ...
  **Z‑6 — `CreateIcon`: the glyph is the KIND, the badge is the ACTION.** All nine `*Plus` geometries carried
  ...
  reasoning: [product-polish-m4-next-session.md](../../docs/history/handovers/product-polish-m4-next-session.md) §5):
  ...
  **Historical pointer for M3/M3b detail:**
  [docs/design/product-polish-m3-next-session.md](../../docs/history/handovers/product-polish-m3-next-session.md) and the
  ...
  **The user's call:** only the elapsed time stays on top; the bar and all statistics move to the **bottom
  ...
  [docs/design/product-polish-m3-next-session.md](../../docs/history/handovers/product-polish-m3-next-session.md).
