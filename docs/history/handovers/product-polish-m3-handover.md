...
| **Suite** | **7310**, zielony w trzech partycjach (**7193 + 63 + 54**). ⚠ Partycja headless ma **siedem** klas — `MetadataTreeVirtualizationProbe` (15b) i `SharedContextMenuFeasibilityProbe` (M3.4b) też są **w filtrze**; `StatusProgressSourcesTests` (M3b.1) **NIE** — nie konstruuje kontrolek Avalonii. **Mierz przed cytowaniem** |
| **Smoke** | czysty |
| **Drzewo** | czyste |
| ⭐⭐ **NASTĘPNY KROK** | ⛔ **BRAMA §13.3** — przegląd czterech powierzchni trwałych JEDNOCZEŚNIE, na żywej bazie, w obu motywach, pod kątem **odbioru wizualnego**, nie zgodności z dokumentem. ✅ **M3b ZAMKNIĘTE W CAŁOŚCI**: M3b.1 (§19.31) · A+B+C (§19.32) · M3b.1d (§19.33) · M3b.2 (§19.34) · M3b.3 przeanalizowane i odłożone (§19.35). Po bramie: podsumowanie zamykające M3 + handover M4 |
| ✅🐞 **WYBÓR DUŻEGO PLIKU DO IMPORTU — NAPRAWIONY (2026-08-04)** | §19.32. ⚠⚠ **Mój pierwszy pomiar odpowiedział na inne pytanie i użytkownik to wychwycił:** wyceniłem *koszt* providera, a objaw („nie odświeża UI", „przeskakuje przy odmalowywaniu") mówił o *wątku*. ⭐ **Koszt tłumaczy, dlaczego coś jest WOLNE; nie tłumaczy, dlaczego UI jest ZABLOKOWANY.** Mechanizm: `Recalculate` startuje łańcuch synchronicznie, a awaity providera kończą się inline (`Task.FromResult`), więc **cały odczyt biegł wewnątrz settera `Source.FilePath`** — zadanie o priorytecie `Render` nie wykonało się ani razu przez 17,8 s. **A:** `RowsFromDimension` czytał `<dimension>` przez akcesor DOM materializujący cały arkusz (**8 546 → 15 ms**), i to **łamało regułę REK‑6 (1) „SAX not DOM", którą ta klasa sama deklaruje**. **B:** trzy wywołania providera poza wątek, kolekcje na Dispatcherze — ⭐ **wzorzec, który ten plik już stosował** w dwóch innych ogniwach. **C:** `IsRecalculating` → „Loading file…", z gaszeniem osłoniętym na wyprzedzenie łańcucha. ⭐⭐ **`data-import.md` §4.7 deklarował „odczyt idzie na wątek tła" od v2 — implementacja tego nie robiła**; zapis w dokumencie projektowym starzeje się tak cicho jak string (#284) |
...
| 🔧 **`EMBERTERN_TREE_DIAG` ZOSTAJE** | Ukryte narzędzie deweloperskie (decyzja użytkownika). To ono znalazło przyczynę po dwóch latach objawu; bez flagi kosztuje zero. ⛔ Nie usuwać, nie wystawiać w UI. ⭐ Sięgać po nie przy **każdym** defekcie o kształcie „przewijanie / zaznaczenie / Dispatcher" |
...
> ✅✅ **ZAMKNIĘTE 2026-08-04 — PRZYCZYNA ZNALEZIONA I NAPRAWIONA. Poniższy opis zostaje jako zapis
> drogi, nie jako otwarta pozycja.** Nie był to ani splice, ani wirtualizacja: gdy jakiś wiersz jest
> zaznaczony, a użytkownik rozwinie dużą kategorię, Avalonia odkłada na Dispatcher
> `SelectingItemsControl.AutoScrollToSelectedItemIfNecessary`, a `VirtualizingStackPanel.ScrollIntoView`
> **nie potrafi skoczyć do nierealizowanego indeksu** — pełznie do celu po jednym wierszu (24 px) na cykl,
> zagładzając priorytet tła. Naprawa: `AutoScrollToSelectedItem="False"` na `SidebarList`.
> Diagnoza z żywego logu: `metadata-refresh-analysis.md` §10–§12; zapis etapowy: `product-polish.md` §19.29.
...
| **R18** | ⭐⭐ **PRZY RÓWNEJ CZYTELNOŚCI WYGRYWA WARIANT GĘSTSZY.** *„EmberTern jest narzędziem dla deweloperów baz danych, więc priorytetem jest ilość informacji widocznych na ekranie bez pogarszania czytelności."* Ratyfikowana 2026‑08‑08 wraz z decyzjami A‑3/B‑1/C‑1/D, **obowiązuje w całym M4**. ⚠ To reguła ROZSTRZYGAJĄCA REMIS, nie mandat do zagęszczania: warunek „bez pogarszania czytelności" jest pierwszy, a jego kryterium jest ekran (**R16**), nie arytmetyka. ⛔ Nie używać jej do uzasadnienia zejścia poniżej zmierzonego minimum — w C‑1 rola stanęła na 30, a nie na arytmetycznych 28, i to jest zgodne z R18, bo 28 nie było „równie czytelne", tylko „bez zapasu" |
...
| ✅ **17b** | **M3b.2** | **ZROBIONE** (§19.34). Ładowanie połączenia na **najwyższym szczeblu** drabinki. ⭐⭐ Lej startu istniał (dwa wejścia, jedna metoda), **lej końca NIE**: `MetadataReady` nie pada przy nieudanym połączeniu, przy rozłączeniu w trakcie ani gdy prefetch rzuci niełapany wyjątek — więc pasek gaśnie **własnym `finally` każdej fazy**, a `MetadataReady` zostaje nietknięte i nieużywane przez pasek. ⚠⚠ **Faza 2 (odtwarzanie zakładek) NIE dostała etykiety: byłaby martwym UI** — odmalowanie następuje PRZED nią, więc napis pojawiłby się po jej zakończeniu; ratyfikowano utrzymanie etykiety fazy 1. ⭐ Faza 3 to **druga** ścieżka z uczciwym procentem (13 kategorii). ⚠ Ręczne odświeżenie świadomie NIE raportuje — pinowane strażnikiem, bo podłączenie „przy okazji" to jedna linia | — |
| ✅ **17a** | **M3b.1 A+B+C** | **ZROBIONE** (§19.32). Wybór dużego `.xlsx` nie blokuje UX: **17 768 → 1 ms**. A — `<dimension>` przez SAX zamiast DOM (8 546 → 15 ms; naprawa odstępstwa od REK‑6 (1), nie optymalizacja) · B — trzy wywołania providera poza wątek UI, **kolekcje zostają na Dispatcherze**, wzorzec już obecny w tym pliku · C — `IsRecalculating` → „Loading file…" / „Reading clipboard…", gaszenie osłonięte na wyprzedzenie. ⭐⭐ **Pierwsze testy jednostkowe `XlsxImportProvider`** (nie miał żadnych) · ⚠⚠ jeden strażnik czyta ŹRÓDŁO, bo DOM i SAX zwracają **tę samą wartość** — żadna asercja na wyniku ich nie odróżni · ⚠ **suita nie dowodzi zejścia z wątku UI**, to rola sondy | — |
| ✅ **17b** | **M3b.1d** | **ZROBIONE** (§19.33). Po QA: postęp importu był w DWÓCH miejscach, a pasek poleceń importu **przycinał** etykiety w trakcie przebiegu. ⭐⭐ Mechanizm: band B to `DockPanel` z `LastChildFill` — elementy dokowane z prawej biorą rozmiar **pierwsze**, a przyciski są ostatnim dzieckiem, czyli poziomym `StackPanel`em, który **się nie zwęża, tylko PRZYCINA** (520 px minimów list + ~400 px postępu w trakcie przebiegu). Decyzja użytkownika: **na górze tylko czas trwania**, pasek postępu i statystyki **do dolnego panelu** — zgodnie z ratyfikowanym podziałem *góra = konfiguracja, dół = wyniki*. ⭐ Wstawione jako **nakładka na pasek zakładek** (wzorzec chevronu), nie własny wiersz: wiersz przesuwałby zakładki w chwili startu importu. ⚠ Korekta faktu, która zmieniła zakres: **„Loading file…" nigdy nie było w górnym pasku** — dublował się stary pasek z etapu I5, nie nowa etykieta | — |
| ✅ **17c** | **M3b.3** | **ZAMKNIĘTE jako przeanalizowane i ODŁOŻONE** (§19.35). **Zero zmian w kodzie — i to jest wynik.** Założenie (rail rozróżnia aktywności kolorem) **zostaje ratyfikowane jako cel**; blokadą jest PALETA, nie decyzja. ⛔ Zmierzone: rail ma **2 px**, severity zajmuje 0° i ~36°, a **wszystkie** istniejące barwy tożsamości siedzą w pasmie **149–215°** i każda para koliduje (9–35°). Pięć rozróżnialnych barw wymagałoby odcieni, których produkt nigdy nie używał. ⭐⭐ **Lekcja: ograniczenie NARZĘDZIA nie jest argumentem za zmniejszeniem WYMAGANIA** — rekomendowałem cięcie liczby kategorii pod obecną paletę, użytkownik odrzucił i miał rację. ⏸ Nowy temat **P‑1** w `color-language.md` §9.2, razem z **P‑2** (`AccentBrush` na railu 2,89:1 w Dark) i **P‑3** (debugger ma DWIE barwy na jeden fakt: chip vs rail) | — |
