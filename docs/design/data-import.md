...
**Status: 🔒 ARCHITEKTURA ZAMROŻONA (v3, z amendmentem I7.5). 🏁 MODUŁ ZAMKNIĘTY I ZAAKCEPTOWANY
(2026-07-27), scalony do `master`.**
Ten dokument opisuje **jak moduł jest zbudowany i dlaczego** — w czasie teraźniejszym. Narracja etapowa
mieszka w **[docs/history/21-data-import.md](../history/21-data-import.md)**.
Szczegóły stanu i to, co zostaje świadomie otwarte poza modułem — blok „📍 STAN MODUŁU" niżej.
...
> **Zamrożenie przetrwało cały moduł: od tego momentu do zamknięcia nie było ani jednej zmiany
> projektowej.** To jest wynik, nie deklaracja — §4.8 postawił nawet dowód obalający („jeżeli nazwane
> profile wymagają zmiany choćby jednego modelu, zamrożenie zostało po drodze złamane") i rachunek się
> zgodził.
>
> - Dokument zmienia się już tylko **w miejscu** i tylko o **stan faktyczny** — nigdy o decyzje projektowe.
> - Odkrycie, które naprawdę podważa projekt, nie jest wymówką do przeprojektowania po cichu:
>   **zatrzymujemy pracę, raportujemy użytkownikowi i czekamy na decyzję**
>   (kontrakt wdrożenia etapowego — `memory/feedback_staged_implementation_contract.md`).
> - Wstawki oznaczone **„(I0)"** pochodzą z pomiaru, nie z projektowania — widać po nich, co jest decyzją,
>   a co wnioskiem z liczby.
...
Dokument jest samowystarczalny: sesja dotykająca modułu zaczyna od niego, **bez ponownej analizy**. Wzorce:
`docs/design/execution-modes-and-export-framework.md` (framework eksportu — lustrzane odbicie tego modułu)
oraz `docs/design/firebird-debugger-implementation-plan.md` (kontrakt wykonawczy).

---

## 📍 STAN MODUŁU

**🏁 Moduł Data Import jest ZAMKNIĘTY i ZAAKCEPTOWANY (2026-07-27).** Wszystkie etapy dostarczone,
gałąź `feat/data-import` scalona do `master`.

Ten dokument jest odtąd **wyłącznie architekturą** — opisuje, jak moduł jest zbudowany i dlaczego, w czasie
teraźniejszym. Cała narracja etapowa (as-built etapów I0–I12, przeglądy wzrokowe, inwentarze testów,
odstępstwa przyjęte w trakcie implementacji, plan etapów) została przeniesiona **w całości** do
**[docs/history/21-data-import.md](../history/21-data-import.md)** — zgodnie z konwencją projektu nic nie
zostało usunięte, tylko przesunięte tam, gdzie mieszka historia.

| | |
|---|---|
| **Architektura** | 🔒 **ZAMROŻONA** (v3) od zamknięcia I0. Zmiany tylko „w miejscu" i tylko o stan faktyczny |
| **Narracja modułu** | [docs/history/21-data-import.md](../history/21-data-import.md) |
| **Archiwum pomiarowe I0** | [data-import-i0-findings.md](data-import-i0-findings.md) — dowody, na których stoją decyzje oznaczone „(I0)" |
| **Weryfikacja na żywo** | `tools/probes/DataImportProbe` (20/20) i `tools/probes/DataImportRunProbe` (33/33) na FB5 — **trzymać i uruchamiać po każdej zmianie writera, mappera lub konwertera** |

### Co obowiązuje po zamknięciu

⛔ **Nie wracamy do kosmetyki tego modułu.** Powrót uzasadnia wyłącznie realny defekt funkcjonalny.

⛔ **Nie upraszczać testów przypiętych w etapach I1–I12** (decyzja użytkownika, podtrzymana przy każdym
etapie). Pełny inwentarz tego, co każdy z nich pilnuje, jest w pliku historii; kilka z nich pilnuje własności,
których naruszenie ujawniłoby się dopiero jako uszkodzone dane u użytkownika — m.in. że każda wartość widziana
przez wnioskownik typów przechodzi przez konwerter na typie, który dla niej zaproponowano, i że `CREATE`
poprzedza pierwszy wiersz (#213).

⚠ **Dokument odzwierciedla architekturę poprawioną w trakcie implementacji, nie pierwotny szkic.** Tam gdzie
implementacja odeszła od szkicu, obowiązuje sekcja architektoniczna niżej, a powód odejścia jest opisany
w historii. Nie „naprawiać" kodu z powrotem do szkicu.

**Otwarte świadomie i celowo poza modułem** — pełna tabela z powodami jest na końcu pliku historii; skrótowo:
sprint UX całego EmberTerna (gęstość kontrolek U4, responsywność U5 i reszta życzeń z przeglądów), wymiana
profili przez `.json`, znacznik „zmodyfikowany" przy profilu, platformowy audyt cichej utraty znaków spoza
charsetu połączenia oraz zawieszenie pełnego zestawu testów (#94/#226/#261). Żadna z tych pozycji nie jest
defektem Data Importu.
...
### 3.8. Uwagi z przeglądów wzrokowych — rozstrzygnięte

Powierzchnia przeszła kilka przeglądów użytkownika (I5, trzy rundy przy I11, plus przegląd domykający).
Wszystkie uwagi są rozstrzygnięte, a ich zapis — co zgłoszono, jak rozstrzygnięto i dlaczego — mieszka
w **[docs/history/21-data-import.md](../history/21-data-import.md)**. Układ, który z nich wynikł, jest opisany
**w czasie teraźniejszym w §3.1** i to on obowiązuje.

Dwie rzeczy warto znać, zanim ktoś tknie ten układ:

⭐ **Podział odpowiedzialności jest zasadą, nie preferencją: góra = konfiguracja importu, dół = wyniki
i artefakty.** Dwa panele biją się o miejsce najczęściej wtedy, gdy jeden jest decyzją, a drugi jej skutkiem —
i wtedy żadna odpowiedź wyrażona w pikselach nie jest dobra. Zanim zmienisz proporcje, sprawdź, czy oba panele
należą do tej samej kategorii.

⛔ **Nie dodawać podłogi (`MinHeight`) wierszowi roboczemu i nie odpowiadać na ciasnotę kolejną sekcją
pionową.** Oba podejścia zostały zbudowane i cofnięte. Pozostała ciasnota jest kwestią **globalnych wysokości
kontrolek** i należy do sprintu UX całego EmberTerna (U4/U5), nie do tego modułu.
...
## 6. Etapy implementacji — wszystkie dostarczone

Moduł powstawał w trzynastu etapach (I0–I12), po jednym na sesję, każdy kończony `build 0/0`, zielonym
zestawem testów i przeglądem użytkownika. **Wszystkie są zamknięte i zaakceptowane.**

Plan etapów wraz z zakresem, kryteriami odbioru i tym, co faktycznie dostarczył każdy z nich, jest
w **[docs/history/21-data-import.md](../history/21-data-import.md)**. Tutaj zostaje tylko to, co z tego
podziału wynika dla architektury i obowiązuje dalej:

- **I0 był blokujący i bezkodowy** — sondy pomiarowe przed projektem, nie po nim. Trzy z jego pomiarów
  poprawiły projekt, zanim powstała pierwsza linia produkcyjna; dowody: [data-import-i0-findings.md](data-import-i0-findings.md).
- **Kolejność etapów szła od rdzenia na zewnątrz** (modele → konwersja i mapowanie → pipeline → Firebird →
  UI → kolejne źródła → profile), więc każde nowe źródło i profile trafiały na gotowy, niezmieniany rdzeń.
  To ta kolejność sprawiła, że filar „jeden pipeline dla każdego źródła" dało się **sprawdzić**, a nie tylko
  zadeklarować.
- **Etapy dokładające źródło nie tknęły pipeline'u** — ani XLSX, ani XLS, ani schowek. Jeżeli kolejne źródło
  kiedyś będzie tego wymagało, to jest sygnał, że łamie §1.4, a nie że pipeline wymaga rozszerzenia.
...
| R13 | **Gęstość jednej powierzchni** na małym ekranie | średnie | nieużywalność w oknie 1366×768 | Sekcje zwijalne (i zwijają się same, gdy kompletne), splittery (dolny panel oraz podział powierzchni roboczej), dolny panel zwijany, podgląd po konwersji i DDL zeszły do zakładek. ⚠ **RYZYKO POZOSTAJE OTWARTE i nie zostało zweryfikowane na 1366×768.** Audyt domykający sprawdził obie palety i tokeny motywu, ale nie rozdzielczość — a przeglądy pokazały, że ciasno bywa już na 1920×1080. To ta sama przyczyna co U4/U5 (globalne wysokości kontrolek), więc weryfikacja małego ekranu należy do **sprintu UX**, nie do tego modułu. |
...
| **Reguła #9** (motywy) | ✅ **Zweryfikowane w audycie domykającym:** zero twardych kolorów, zero lokalnych pędzli, zero `{StaticResource}` na pędzlu, zero lokalnych stylów w widokach modułu, 13/13 tokenów obecnych w OBU paletach — **przypięte testem** (`DataImportSurface_EveryThemeToken_ResolvesInBothPalettes`), bo `{DynamicResource}` rozwiązuje się dopiero w czasie działania. `Themes/ControlStyles.axaml` nietknięty od I5. ⚠ Nie obejmuje 1366×768 — patrz R13. |
