...
**Etap order:** **✅ ① Preview + Full** → **✅ ② Full smart-guarding/large-result polish** → **✅ ③ Export Framework (core + CSV/TXT/Clipboard, + `[Export all…]`)** → **✅ ④ XLSX** → **✅ ⑤ Consumers: Activity Monitor + Table Data + View Data**. Then, later: Benchmark, Export Script, remaining grid consumers (Session Manager, Performance grids).

**Etap 4 — XLSX (SHIPPED 2026-07-08).** New `EmberTern.Export.Office` project (the one place a NuGet dep is allowed) with **`DocumentFormat.OpenXml` 3.1.0** — a streaming `OpenXmlWriter` (Risk #2's recommended option over ClosedXML for large exports). `XlsxExporter : IExporter` writes typed cells (invariant numbers → no CSV separator trap; real date cells via one date style; boolean cells; BLOB → `(BLOB)`; NULL → empty; else inline string), splits at Excel's 1,048,576-rows/sheet limit (no data loss). Excel is now the dialog's **default** format (headline; file-only). `IExportSink` gained a `Stream` surface alongside `Writer` (realizing B.2's "TextWriter / Stream" — additive, text exporters unchanged); `FileExportSink` is read+write (the OOXML package reads back the zip directory).

**Etap 5 — Consumers (SHIPPED 2026-07-08).** Two reusable adapters + per-module builders, no export logic duplicated: **`RowBufferExportSource`** (materialized VM-row grids, no re-fetch) → **Activity Monitor** (`BuildExportSource`, reusing its `FilterColumns` + `ProjectRow`; CurrentView=filtered / AllRows=buffer / SelectedRows=selection; default CurrentView); **`ServerPagedExportSource`** (current page + page-by-page re-fetch honouring filter+order) → **Table Data** + **View Data** (`BuildDataExportSource`; default AllRows). Entry points per grid = a toolbar Download icon + a right-click "Export…", opening the shared dialog via `ExportDialog.LaunchAsync(host, source, defaultScope)`. **Deferred (not "data grids"):** Session Manager (Sessions/Transactions) + Performance grids — same `RowBufferExportSource` pattern if wanted later.
