...
- **Etap 2 — Full: smart guarding + large-result UX (refine, do NOT re-add Full).** Full already exists (Etap 1). This etap *develops its behaviour*: the **smart soft confirm threshold** (~250k, mid-stream — warn before the hard ceiling — or up front when the source has a cheap estimate; A.6.1); the **distinct, refined ceiling-hit bar** messaging; polished UX for very large results; refined full-set timing presentation; any further full-read improvements. *No new capability — the capability shipped in Etap 1; Etap 2 makes it smart about size.*
...
- **Etap 2 refines Full, does not add it** → smart *soft* threshold (~250k, mid-stream / up-front-when-estimate), refined ceiling-hit messaging, large-result UX. The *capability* is Etap 1; the *intelligence* is Etap 2. (A.6.1)
...
**Etap order:** **① Preview + Full (complete)** → **② Full smart-guarding/large-result polish** → **③ Export Framework (core + CSV/TXT/Clipboard, + `[Export all…]`)** → **④ XLSX** → **⑤ Activity Monitor**. Then, later: Benchmark, Export Script, remaining grid consumers.
