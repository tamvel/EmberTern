...
| **Suite** | **7249**, zielony w trzech partycjach (**7134 + 61 + 54**). ⚠ Partycja headless ma teraz **sześć** klas — doszła `MetadataTreeVirtualizationProbe` (krok 15b) i jest **w filtrze**. ⚠ Ten wiersz podawał kiedyś **7138 (7032 + 52 + 54)**. **Mierz przed cytowaniem** |
| **Smoke** | czysty |
| **Drzewo** | czyste |
| ⭐⭐ **NASTĘPNY KROK** | **M3.4b — przegląd menu kontekstowych** (pozycja 16 w planie §10). Potem M3b → ⛔ brama §13.3 |
...
| ⭐⭐ **16** | **M3.4b** | **← TU ZACZYNASZ.** Przegląd menu kontekstowych. ⭐ Punktem odniesienia jest menu zakładki z M3.3c: ikony przez `{app:MenuIcon}`, gesty przez `{app:CommandGesture}`, **każda pozycja z własnym `CanExecute`**. Sprawdź, czy 32 istniejące menu trzymają ten poziom | — |
