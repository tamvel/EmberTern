...
| **Suite** | **7229**, zielony w trzech partycjach (**7118 + 57 + 54**). ⚠ Ten wiersz podawał wcześniej **7138 (7032 + 52 + 54)** — wartość sprzed rundy poprawek odbiorczych (`85c8747`, §21), która dołożyła 90 testów. **Mierz przed cytowaniem** |
| **Smoke** | czysty |
| **Drzewo** | czyste |
| ⭐⭐ **NASTĘPNY KROK** | **M3.3b — dwa tryby paska zakładek + dwie preferencje** (pozycja 13 w planie §10): `TabStripMode` (`MultiRow` \| `SingleRow`, domyślnie MultiRow) i `TabStripMaxRows` (1–10, domyślnie 3) + wiersze w Settings Center. Model: §8.2. ⚠ **`CurrentSchemaVersion` ZOSTAJE 2** (R‑4, addytywnie) · wzorzec preferencji numerycznej: `settings-center.md` §17.4/§17.4a (`PreferenceRange`, commit na blur/Enter, digits‑only na tunelu, ⚠⚠ kontrolka niesie **swój WIERSZ** jako `DataContext`) · lista z filtrowaniem dla trybu B to **istniejący `SearchableComboBox`**. Potem M3.3c → M3.4a → M3.4b → M3b → ⛔ brama §13.3 |
...
| ⭐⭐ **13** | **M3.3b** | **← TU ZACZYNASZ.** Dwa tryby + preferencje (`TabStripMode`, `TabStripMaxRows`) + wiersze w Settings Center | ⚠ preferencje |
