...
| ⭐⭐ **16b** | **M3.4b cz. 2** | **← TU ZACZYNASZ.** Właściwy przegląd menu. Stan wejściowy **zmierzony**: 32 `ContextMenu`, 154 `MenuItem`, **140 z ikoną (14 bez)**, 27 z gestem z katalogu. ⭐ Punkt odniesienia to menu zakładki z M3.3c: ikona przez `{app:MenuIcon}`, gest przez `{app:CommandGesture}`, **własne `CanExecute` na każdej pozycji**. ⚠ Sprawdź, czy 14 pozycji bez ikony to świadome kwalifikatory zakresu (jak w M3.3c), czy dryf | — |
