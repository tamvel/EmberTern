...
| **Suite** | **7135**, zielony w trzech partycjach (**7032 + 49 + 54**) |
| **Smoke** | czysty |
| **Drzewo** | czyste |
| **NASTĘPNY KROK** | ⏸ **NAJPIERW QA WIZUALNE K2** (destrukcja 🟡 → 🔴, `product-polish.md` §19.16) — ⛔ **nie zaczynaj K3 przed akceptacją użytkownika.** Punkt do obejrzenia: dwie czerwone ikony obok siebie w nagłówku Saved Queries (§19.16.4). Po akceptacji: **K3** (Edytuj → ⚪). ⭐ Wykonane: **K1** (§19.15, `ActionRunBrush`, zero zmiany na ekranie) · **K2** (§19.16). ⛔ **Projekt języka jest ZAAKCEPTOWANY — nie projektuj go ponownie, wykonuj §11 krok po kroku.** ⛔ **R14: jeden krok = jedna iteracja = jedno obejrzenie na żywo** |
