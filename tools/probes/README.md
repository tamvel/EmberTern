...
| `DataImportWriteProbe` | The **Data Import / etap I0** write-path gates ([data-import-i0-findings.md](../../docs/design/data-import-i0-findings.md)): how fast is a prepared `INSERT` + re-bind vs `FbBatchCommand` vs the naive per-row command, and what does committing every N cost? **Does a batch tell us WHICH row failed** (the blocking question — without it the batch path cannot fill the error report)? Which SQLSTATE/GDS codes identify each error class? Is an over-long string an error or a silent truncation? And the decisive one: when a value carries a character the **connection** charset cannot represent, does it fail — or is it **silently substituted**? | **Answered** — batch is 16× faster (~121 000 rows/s at 500/chunk) **and** reports the failing row 1:1 by batch index; commit frequency is nearly free; BLOBs pass through a batch; three distinct error classes share `ErrorCode` 335544321 ⇒ the mapping must read the **GDS vector**; an over-long string is rejected, never truncated; **a character outside the connection charset is silently written as `?`, even into a UTF8 column** ⇒ local validation is a §0 requirement. **Keep until I4** (its B/E/C phases are the spec `FirebirdImportWriter` must reproduce), then delete. |
