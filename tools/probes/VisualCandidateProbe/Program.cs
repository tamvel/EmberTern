...
internal static class Program
{
    ...
    public static void Main(string[] args)
    {
        ...
        if (args.Length > 0 && args[0] == "dbprops")
        {
            DatabaseProps.Run(outDir);
            Console.WriteLine("OK");
            return;
        }
        ...
        {
            Severity.Run(outDir);
            foreach (var file in Directory.GetFiles(outDir, "m5c-*.png").OrderBy(f => f))
            {
                Console.WriteLine(file);
            }
        ...
        if (args.Length > 0 && args[0] == "typo")
        {
            ...
            foreach (var file in Directory.GetFiles(outDir, "m4t-*.png").OrderBy(f => f))
            {
                Console.WriteLine(file);
            }
