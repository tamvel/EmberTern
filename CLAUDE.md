...
- **Security**: connection passwords encrypted at rest via **DPAPI CurrentUser** (only `Password` is encrypted; `connections.json` is now a versioned v1 container with `ProtectedPassword`). Legacy plaintext files auto-migrate on first load; writes are atomic. Reusable `SecretProtector` seam (Core) + `DpapiSecretProtector` (App) are the foundation for upcoming ApplicationSettings + config export/import (not yet built). DPAPI ciphertext is per-user/per-machine → not portable; cross-machine export will need a passphrase path.
...
- **Branch state**: working on master. Latest milestone: **Security — connection-password encryption at rest (DPAPI)**. Next planned workstream (not started): user-settings persistence — ApplicationSettings store, grid/column-width profiles, layout persistence, and config export/import (the export path will need a passphrase-based portable encryption, separate from the at-rest DPAPI). C2 (second read-only metadata connection) remains analysed-but-deferred.
