...
- **Audit update (2026-06-18, R3):** root-caused — this is **not a bug, it is the designed Phase A behavior** bleeding into a UI built for the old staged model. `AddFieldAsync`/`DropFieldAsync`/`MoveField*` now call `Execute*Async` → `_ddlExecutor.ExecuteAsync` → autonomous auto-commit immediately. The Compile button gate `CanCompile => _ddlExecutor is not null && HasPendingChanges` is permanently false because **nothing populates `PendingChanges` in production anymore** (`AddMovePending` is test-only). The `PendingChanges`/`CompileAsync` machinery, the inert metadata Commit/Rollback buttons, and the unused `transactionService` param on `FirebirdDdlExecutor` are all dead/vestigial.
- **DECISION (user, 2026-06-18): keep Phase A — structural changes apply immediately + auto-commit, uniformly across ALL metadata objects (tables, fields, indexes, keys, views, procedures, functions, triggers, packages). Transactions are for DATA only (Data lane).** The dead staged-Compile workflow (`PendingChanges`, the inert Compile button from the old model, the unused metadata Commit/Rollback UI, and any code assuming staged structural edits) is to be **removed** — but **deferred to a future session**, NOT now. This session does F1 + Issue-B diagnostics only; the lock/transaction architecture is not touched until the MON$ evidence for Issue B is in.
