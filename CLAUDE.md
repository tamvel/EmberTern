...
- **Tests**: 407 / 407 passing (includes 2 headless Avalonia binding probes; test project references `Avalonia.Headless` 12.0.3).
- **App**: builds, launches, exits cleanly. SQL editor: autocomplete (Ctrl+Space, 3+ char auto-trigger, `ALIAS./TABLE.` column completion) — **case-preserving on insert (nagl→nagl_table; NAGL→NAGL_TABLE; mixed→catalog form) and IBExpert-style 2-column display (kind label + "NAME : TYPE" for columns / name for objects)**, formatter lowercases keywords + identifiers, syntax highlighting swaps on theme toggle, selection brush matches the rest of the app, light-theme accent blue, double-click on a loaded metadata object name opens its DDL tab. **Saved queries can be renamed in place** — full-row double-click, right-click → Zmień nazwę / Usuń, or hover-visible `✎` icon button — and are persisted via the existing close-time CaptureWorkspace path. **Connections can be organized in folders** via a 📁 button in the titlebar; right-click a folder for Dodaj połączenie / Zmień nazwę / Usuń katalog (delete moves children back to root); right-click any connection for Sortuj węzły → Rosnąco/Malejąco (siblings sorted by Name; persisted in folders.json). The titlebar `+` add-connection button also auto-scopes to the folder context when a folder (or a folder-member connection) is selected in the tree. **Tables open as a 6-sub-tab TableDetailTab (Pola / Ograniczenia / Indeksy / Dane / Opis / DDL)** — Pola now shows **14 columns including PK / FK / Unique icons + Domain + FK Tabela + Computed + Charset**, with autosize (`ColumnWidth="Auto"`) and **per-column sort on header click** (every column carries explicit `SortMemberPath` — mandatory under compiled bindings). 1-based field position, zero scale blanked, font 11, tight 22 px row pitch, text-checkmark booleans (✓/empty), Dane shows first 200 rows in a dynamic grid, Opis surfaces the table's RDB$DESCRIPTION. TableDetail tabs survive restart and lazy-load on first activation. **Readers never open their own transaction** — they attach to the user's working tx when active, otherwise the managed driver runs each SELECT in an implicit auto-committed read tx. FbConnection is single-threaded — command execution is serialized application-wide via `FirebirdConnectionService.CommandLock` across readers, executor, and TransactionService. Errors in any single sub-step surface in the view itself (Messages panel is hidden on non-Query tabs). Toolbar buttons + Saved Queries panel + bottom Results/Messages panel all hide on non-Query tabs.
- **Branch state**: working on master. All prior milestones plus **TableDetail Pola — PK/FK/Unique columns + sorting + autosize** (3 template columns with PK/FK/UNQ SVG icons gated by `IsPrimaryKey`/`IsForeignKey`/`IsUnique` bools on `FieldInfo`; +3 text columns Domain/FK Tabela/Charset; FieldsSql joins `RDB$INDEX_SEGMENTS`/`RDB$RELATION_CONSTRAINTS`/`RDB$CHARACTER_SETS`; `BaseTypeName` strips parens off Type; `NormalizeDomain` filters RDB$xxx anonymous backing domains; `WarningColor`/`WarningBrush` added to both themes; sort + reorder enabled on all 3 sub-grids with explicit `SortMemberPath` on every column; explicit `Width="N"` stripped so `ColumnWidth="Auto"` does the work; **converter `ConvertBack` regression fix** — `ZeroToEmptyConverter` + `BoolToCheckmarkConverter` switched from `throw NotSupportedException` to `return BindingOperations.DoNothing` so VS no longer breaks on user-unhandled exception when opening a restored TableDetail tab; **double-click-on-gripper autofit not shipped** — Avalonia 12.0.3 lacks it natively, reflection-based workaround didn't re-fit cleanly, deferred to a future Avalonia upgrade). Plus all prior shipped: **Tree drag-and-drop**, **Tree expand-state persistence**, **auto-expand-on-connect fix**, **connection-error mapping reduced to the raw server message**.
