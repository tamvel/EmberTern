...
- **Branch state**: working on master. Latest milestone: **settings.dat file container — future-proofing** (container header + explicit encryption scheme + 3-axis downgrade protection + migration ladder; full backward compat, real file migrated in place). Deferred (architecture ready): click-to-change profile from the chip (flyout + `SetData/MetadataProfile`, profile read at begin time so a live switch is safe). Next planned workstreams (not started): config **export/import** — now mostly unblocked by the container (add an `aes256-passphrase` `SecretProtector`, register it in `ResolveProtector`, write that scheme into the header for the exported file); wire `GridProfiles` (column width/order/auto-fit memory — field set already in the schema) and `AppearanceSettings` to real consumers.
