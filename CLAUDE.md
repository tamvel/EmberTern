...
- **Tests**: 924 / 924 passing.
- **Security / settings**: all user settings live in **one whole-file DPAPI-encrypted file** `%AppData%\EmberTern\settings.dat` (`ApplicationSettings` aggregate: Connections + Folders + Workspace + UserSettings). `ConnectionProfileStore` / `FolderStore` / `WorkspaceStore` are now section facades over a shared `ApplicationSettingsStore` (read-modify-write per section). First launch migrates the legacy `connections.json` / `workspace.json` / `folders.json` (v0 or v1 connections) into `settings.dat` and deletes them. `UserSettings.GridProfiles` (GridId/ColumnWidths/ColumnOrder/AutoFitColumns) + `AppearanceSettings` are persisted stubs for upcoming milestones. DPAPI ciphertext is per-user/per-machine → not portable; cross-machine export will need a passphrase path (seam ready: inject a passphrase-based `SecretProtector` into `ApplicationSettingsStore`).
...
- **Transactions (C2 shipped)**: **two independent lanes per connection** — Data (connection #1: SQL Editor F5, data preview/edit) and Metadata (connection #2: DDL from the structure editor, Shift+F5, metadata browsing). Each lane has its own **transaction profile** (`DataTransactionProfile` / `MetadataTransactionProfile`, both default Read Committed), its own working transaction, and its own Commit/Rollback. Profiles map to explicit TPB; default `write + read_committed + rec_version + nowait` (IBExpert-matching), read at begin time. Browsing runs on the metadata attachment so it no longer pins objects in the data working tx (the original IBExpert-blocking bug). `F5`=Data, `Shift+F5`=Metadata, with an explicit "Executed via X profile (Y)" message per execute. Dual title-bar chips (`Data:` / `Meta:`) + dual Commit/Rollback + two-group transaction bar. Metadata attachment opens best-effort; degrades to aliasing the data lane if the second attach fails. `EMBERTERN_TX_DIAG=1` logs real server-side TPB (per lane) to `%TEMP%\EmberTern-debug.log` on every begin.
- **Branch state**: working on master. Latest milestone: **C2 — second metadata connection + Data/Metadata transaction-profile split**. Next planned workstreams (not started): wire `GridProfiles` (column width/order/auto-fit memory — field set already in the schema) and `AppearanceSettings` to real consumers; config **export/import** (inject a passphrase-based `SecretProtector` into `ApplicationSettingsStore` for a portable file).
