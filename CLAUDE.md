...
- **Tests**: 383 / 383 passing (includes 2 headless Avalonia binding probes; test project now references `Avalonia.Headless` 12.0.3).
...
- **Branch state**: working on master. All prior milestones plus **Tree drag-and-drop** (Connection→Folder Into; Connection/Folder reorder Before/After; pointer-event driven, 8 px threshold, accent-blue drop-target overlay) and **Tree expand-state persistence** (FolderState.ExpandedNodeIds HashSet + ExpandStateInitialized migration flag; ReloadConnections restores from set; on-change writes save immediately under a suppress flag; CreateFolder seeds new folders so default-expanded survives restart; drag/drop no longer collapses the tree because the rebuild path now restores) and an **auto-expand-on-connect fix** (real root cause proven by a headless probe: the three `x:DataType`-typed `TreeViewItem` styles all matched every container and the last one clobbered `IsExpanded` via a failed cast — replaced with a single `ReflectionBinding` container style; all the Dispatcher/toggle workarounds removed; RestoreExpandState asymmetric — connections only-set-true, folders verbatim). Plus **connection-error mapping reduced to the raw server message**: `MapErrorMessage` always returns `"Could not connect to {endpoint}: {ex.Message}"` — no hints, no categorization, no chain scanning. All the interpretation logic (the Legacy_Auth / Srp hint, the login/password/plugin heuristics, `MapFbException` / `MapFbErrorText` / `MapSocketError` / `FindInner` / `Sanitize`) was deleted because it misfired on unrelated failures. The server message is authoritative.
