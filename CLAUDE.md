...
- **Tests**: 438 / 438 passing (includes 2 headless Avalonia binding probes; test project references `Avalonia.Headless` 12.0.3).
- **App**: builds, launches, exits cleanly. SQL editor unchanged from the prior milestone. Sidebar unchanged. **TableDetail tab** opens as a 6-sub-tab view (Pola / Ograniczenia / Indeksy / Dane / Opis / DDL). **Ograniczenia** is now a nested TabControl with 4 sub-tabs (Primary Key / Foreign Keys / Check / Unique) — sub-tab headers carry counters (e.g. `"Foreign Keys (3)"`) and grey out when the kind has no rows. FK sub-tab splits the ON UPDATE / ON DELETE rules into separate columns; PK / FK / Unique each show the backing index's Sort direction (Ascending / Descending). **Indeksy** has 9 columns: type icon (PK/FK SVG), Name, Fields, Expression, Unique, Descending, PK, Active, Statistics (right-aligned). All TableDetail column headers and constraint sub-tab labels are English (main tab names — Pola / Ograniczenia / Indeksy / Dane / Opis — stay Polish per design). Zero inline strings in XAML; all UI strings flow through `UiStrings.cs`.
- **Branch state**: working on master. All prior milestones plus **TableDetail Ograniczenia sub-tabs**, **TableDetail Indeksy extension + UI English-ization**.
