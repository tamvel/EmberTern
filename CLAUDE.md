...
| **`docs/design/product-polish-m4-next-session.md`** | ⭐⭐ **THE STARTUP PROMPT for the next Product Polish session — paste it and go.** State after the §13.3 gate + M3.5 · what to read and in what order · the eight rules that decided most often · ⭐ three lessons from the gate that are not in the rules · what M4 covers and ⭐ **the three RATIFIED direction decisions D‑M4‑1…3 (2026-08-04) — start from them, do not re-ask them** · what is forbidden · the mandatory closing order. ⚠ **Updated 2026-08-08: M4 HAS STARTED and its first item — the density group — is closed and accepted**, so its §5 is now a RECORD of ratified direction, not a to-do list; the header states what comes next (the rest of the collision register, which is now one question about TYPOGRAPHY). | **First, at the start of every M4 session.** |
