...
| **`docs/design/localization-app-stage-handover.md`** | ⭐⭐ **THE STARTUP DOCUMENT for the next session — read it first and start from it.** Entry state (branch/HEAD/remotes/build/test/smoke), what the App stage closed, what was deliberately left, the exact D‑3 recipe for migrating a Core producer, the binding rules for the new session, and the open user decisions. | **First, at the start of the next localization session.** |
...
- **🔒🌍 LOCALIZATION / APP — ETAP ZAMKNIĘTY I PRZYGOTOWANY DO ODBIORU (2026-08-09).** Gałąź
