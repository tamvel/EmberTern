...
136. **Incremental type-ahead = match the growing buffer from the TOP of the ordered index each keystroke, not "next from current selection".** Anchoring the search on the current selection (with inclusive/exclusive cycling) reads as independent per-char jumps and depends on the async-applied selection from the previous keystroke (a race). Matching the whole buffer from index position 0 every time makes `K→KO→KON→KONT→KONTR` deterministically converge on the same item, needs no anchor/last-match state, and is immune to the selection round-trip. Drop the "repeat-a-letter-to-cycle" feature unless explicitly asked — it conflicts with incremental refine. **Caveat (user feedback):** matching from the top of a GLOBAL index means a keystroke can jump to a different category than the one you're in (e.g. from Procedures to Tables). That's accepted for now but is NOT the IBExpert feel — see the deferred "contextual type-ahead" task below. (Note this slightly tensions with #136's "from the top" stance: the *incremental-buffer* part stays; the *search-scope* part should become current-position-relative, not global.)

### Deferred — IBExpert-style contextual type-ahead search (future task, NOT in this branch)

The shipped type-ahead is **incremental** (one growing buffer — good, keep it) but searches a **single global index of every object across all categories**, so it can jump out of the current category (Procedures → Tables). The user accepts this for the merged branch but wants a follow-up task to make it **contextual**, matching IBExpert / Windows Explorer / VS Solution Explorer:
- start the search from the **currently selected** node,
- search the **visible tree from the current position** forward,
- **wrap** to the top when it reaches the end,
- do **not** treat the whole database as one flat global index.

Design note for whoever picks this up: this means reintroducing an **anchor** (the current selection / last match) and searching forward-from-anchor over the *visible/navigable* order, while KEEPING the incremental growing-buffer semantics (don't regress to per-char jumps — gotcha #136) and KEEPING the lazy name cache so a match still resolves into a never-expanded category by loading only its path. The current `ResolveTypeAheadAsync` + `BuildFullTypeAheadIndexAsync` + `FindFirstMatch` are the seam to evolve: `FindFirstMatch` becomes find-from-anchor-with-wrap, and "visible tree" scoping decides whether to descend into collapsed-but-cached categories. Behaviour-test the anchor + wrap with the pure finder before wiring the View.
...
- **Tests**: 1402 / 1402 passing.
- **Object Explorer (metadata tree)**: lazy **COUNT-only load on connect** (category labels show `Tables (2356)` etc. immediately; full leaf lists load on first expand). **Filter** (debounce ~350 ms) is IBExpert-style: shows per-category **match counts** (`Views (1)`) and **hides zero-match categories**, never auto-expands anything (counts for un-expanded categories come from a session object-name cache). **Type-ahead** is true incremental search over the FULL metadata (one growing buffer `K→KO→KON…` matched from the top of the tree; finds objects in never-expanded categories via the name cache, loads only that category, expands only the path, selects + scrolls). **Known**: because it matches a single global index, a keystroke can jump to a different category than the current one — accepted for now; an IBExpert-style **contextual** variant (search from current selection, visible-tree, wrap) is a deferred follow-up task (see milestone). Both features share a lazily-built session name cache (strings, not VMs; invalidated on disconnect/reload/refresh). Still on Avalonia `TreeView` + nested `VirtualizingStackPanel`; the residual scroll-extent jump on very large expanded categories is a control limitation — `TreeDataGrid` migration is parked for a future sprint (see the "Object Explorer performance — Etap 1" milestone + its follow-up). `EMBERTERN_PERF_DIAG=1` logs connect/expand timings + heap to `%TEMP%\EmberTern-debug.log` for before/after measurement.
