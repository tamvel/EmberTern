...
- **Branch state**: working on master. All prior milestones plus **Folders + saved queries follow-up fixes shipped** (saved-query rename full-row hit area + right-click + hover ✎; "Dodaj połączenie" on folder right-click + titlebar `+` auto-detects folder context; sort scope verified — folder sort doesn't touch root and vice versa, with regression tests pinning). Plus **connection-error mapping reduced to the raw server message**: `MapErrorMessage` always returns `"Could not connect to {endpoint}: {ex.Message}"` — no hints, no categorization, no chain scanning. All the interpretation logic (the Legacy_Auth / Srp hint, the login/password/plugin heuristics, `MapFbException` / `MapFbErrorText` / `MapSocketError` / `FindInner` / `Sanitize`) was deleted because it misfired on unrelated failures. The server message is authoritative.
