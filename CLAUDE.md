...
| **`docs/design/product-polish-m4-migration-next-session.md`** | ⭐⭐ **THE STARTUP PROMPT for the next Product Polish session — paste it and go.** State after BOTH M4 decision blocks (density + typography), the ratified decisions that must not be re-opened, what screen migration actually means now that the frame is settled (two ceilings with numbers: 95 icon-size literals, 36 `FontSize`), the three things that surprise if you do not know them, the order/limits for M4.1–M4.4, and the mandatory closing order. | **First, at the start of every M4 migration session.** |
...
- **🔘⏸ M4.3c · `Button.seg` — ZAIMPLEMENTOWANY, CZEKA NA QA (2026-08-09).** ⛔ Nie commitowane.
...
- **🖥⏸ M4.3 (M4.3b) · DEBUGGER · TRACE · SESSION · SECURITY · PERFORMANCE — ZAIMPLEMENTOWANY, ODEBRANY PO QA
  UŻYTKOWNIKA (2026-08-09).** ⛔ Nie commitowane i nie pushowane — cały M4.3 zamyka JEDEN commit po odbiorze
  M4.3c (decyzja użytkownika).
...
- **Build**: 0 warnings / 0 errors (`TreatWarningsAsErrors=true`). **Tests**: **8334, MEASURED 2026-08-09**
  (Product Polish through **M4.2b** + M4's density decision + typography block + the stabilization sprint
  S-1…S-6 + the grid consistency sprint + the Firebird language completeness sprint incl. its QA round).
  Green in the three documented partitions (**8182 + 97 + 55**).
