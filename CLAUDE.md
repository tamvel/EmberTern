...
**New logo + assets** ([src/EmberTern.App/Assets/Branding/](src/EmberTern.App/Assets/Branding/)). Official logo is now a smooth-crested tern head (the prior jagged-crest variant read as noise at thumbnail size). Source supplied by the user as a genuine RGBA transparent PNG (`ptak_logo_przezroczyste_tlo_v3.png`, 877×808). Regenerated in place (filenames unchanged → zero reference changes): `logo.png` = pristine full-res transparent master; `EmberTern_logo.png` = 256×256; `EmberTern.ico` = 16/24/32/48/64/256, all 32bpp; every asset transparent.

- **Fit framing (no crop).** `logo.png` is the pristine full master. The square logo + every `.ico` frame **fit the WHOLE artwork** into the square — content-bbox centred, scaled to the larger dimension with a ~2% margin, **nothing clipped** (the bird is wider than tall → ~6% vertical gap is accepted). The user supplies an already-tightly-cropped source, so the generator must NOT add its own bleed/crop: an earlier "balanced ~3% bleed" pass ate the tail feathers off the right edge and had to be redone. Pipeline: in-session PowerShell + System.Drawing (alpha-bbox → centred square window sized to `max(w,h)*1.04` → `HighQualityBicubic` → hand-assembled ICO: `ICONDIR` + `ICONDIRENTRY`×N + PNG payloads, width/height byte 0 for the 256 frame).
