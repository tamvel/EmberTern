...
- **Tests**: 1416 / 1416 passing.
- **Object Explorer (metadata tree)**: lazy **COUNT-only load on connect** (category labels show `Tables (2356)` etc. immediately; full leaf lists load on first expand). **Filter** (debounce ~350 ms) is IBExpert-style: shows per-category **match counts** (`Views (1)`) and **hides zero-match categories**, never auto-expands anything (counts for un-expanded categories come from a session object-name cache). **Type-ahead** is now **contextual** (IBExpert/VS-style): the incremental growing buffer (`K→KO→KON…`, ~1 s idle reset) searches **forward from the current selection, inclusive, with wrap** — a fresh `K` on a selected `KONTRAHENCI` STAYS there (checks current first, doesn't advance), and a keystroke no longer jumps the user into a different category unless there's genuinely no later match (`FindFromAnchor` + `FindAnchorIndex`; growing buffer anchors on a race-safe `_typeAheadAnchor`, fresh buffer re-reads `tree.SelectedItem`). It still reaches objects in never-expanded categories via the name cache (loads only the matched category, expands only the path, selects + scrolls). Both features share a lazily-built session name cache (strings, not VMs; invalidated on disconnect/reload/refresh). Still on Avalonia `TreeView` + nested `VirtualizingStackPanel`; the residual scroll-extent jump on very large expanded categories is a control limitation — `TreeDataGrid` migration is parked for a future sprint (see the "Object Explorer performance — Etap 1" milestone + its follow-up). `EMBERTERN_PERF_DIAG=1` logs connect/expand timings + heap to `%TEMP%\EmberTern-debug.log` for before/after measurement.
