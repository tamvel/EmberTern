...
- **Tests**: 1401 / 1401 passing.
- **Object Explorer (metadata tree)**: lazy **COUNT-only load on connect** (category labels show `Tables (2356)` etc. immediately; full leaf lists load on first expand), filter **debounce ~350 ms**, **local filter** (auto-expands only loaded matching groups, never force-loads/expands unloaded categories), and **IBExpert-style type-ahead** (typed letters jump to the next matching node, expand its path, select+scroll). Still on Avalonia `TreeView` + nested `VirtualizingStackPanel`; the residual scroll-extent jump on very large expanded categories is a control limitation — `TreeDataGrid` migration is parked for a future sprint (see the "Object Explorer performance — Etap 1" milestone). `EMBERTERN_PERF_DIAG=1` logs connect/expand timings + heap to `%TEMP%\EmberTern-debug.log` for before/after measurement.
