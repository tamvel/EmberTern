...
- **Balanced framing (~3% bleed).** `logo.png` stays the uncropped master. The square logo + every `.ico` frame use a content-bbox-centred square window sized so only ~3% of the beak tip / tail-feather tips bleed off each horizontal edge (the bird is wider than tall → ~3.6% vertical gap), maximising optical mass without meaningfully clipping. This was chosen over fit-to-box (more empty space, looks small beside VS/Rider/DataGrip) and fill-to-height (~6% clip). Pipeline unchanged in spirit: in-session PowerShell + System.Drawing (alpha-bbox → centred square window → `HighQualityBicubic` → hand-assembled ICO: `ICONDIR` + `ICONDIRENTRY`×N + PNG payloads, width/height byte 0 for the 256 frame).
